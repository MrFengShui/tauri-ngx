 ...
 */
@Injectable()
export class RecursiveBatcherMergeSortService extends AbstractBinarySortService {
    ...
    protected async mergeByAscent(source: SortDataModel[], lhs: number, rhs: number, dist: number, times: number, callback: (param: SortStateModel) => void): Promise<number> {
        ...
        if (span < rhs - lhs + 1) {
            ...
            for (let i = lhs + dist; i + dist < rhs + 1; i += span) {
                times = await this.exchange(source, source[i].value > source[i + dist].value, i, i + dist, PRIMARY_COLOR, SECONDARY_COLOR, ACCENT_COLOR, times, callback);         
            }
        } else {
 ...
 */
@Injectable()
export class IterativeBatcherMergeSortService extends RecursiveBatcherMergeSortService {
